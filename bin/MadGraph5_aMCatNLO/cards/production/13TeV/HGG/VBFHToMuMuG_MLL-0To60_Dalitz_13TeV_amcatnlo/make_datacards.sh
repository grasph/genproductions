...
cat > $d/vbfH${MH}_NLO_HtoMuMuGamma_customizecards.dat <<EOF
set param_card mass 25 ${MH}.
EOF

cat > $d/vbfH${MH}_NLO_HtoMuMuGamma_extramodels.dat <<EOF
...
cat > $d/vbfH${MH}_NLO_HtoMuMuGamma_FKS_params.dat <<EOF
! ==========================================================================
...
cat > $d/vbfH${MH}_NLO_HtoMuMuGamma_madspin_card.dat <<EOF
...
cat > $d/vbfH${MH}_NLO_HtoMuMuGamma_proc_card.dat <<EOF
import model loop_sm-no_b_mass

generate p p > h j j \$\$ w+ w- z [QCD]

output vbfH${MH}_NLO_HtoMuMuGamma -nojpeg
EOF

cat > $d/vbfH${MH}_NLO_HtoMuMuGamma_run_card.dat <<EOF
*******************                                                 
# Running parameters
#*******************                                                 
#
