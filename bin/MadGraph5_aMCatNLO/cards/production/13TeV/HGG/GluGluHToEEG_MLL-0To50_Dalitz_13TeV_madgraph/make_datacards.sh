...
d=GluGluHToEEG_M${MH}_MLL-0To50_Dalitz_13TeV_madgraph

mkdir "$d"

cat > $d/ggH${MH}_012j_NLO_FXFX_HtoElElGamma_customizecards.dat <<EOF
set param_card mass 25 ${MH}.0
EOF
cat > $d/ggH${MH}_012j_NLO_FXFX_HtoElElGamma_extramodels.dat <<EOF
...
EOF
cat > $d/ggH${MH}_012j_NLO_FXFX_HtoElElGamma_madspin_card.dat <<EOF
...
launch
EOF
cat > $d/ggH${MH}_012j_NLO_FXFX_HtoElElGamma_proc_card.dat <<EOF
...
output ggH${MH}_012j_NLO_FXFX_HtoElElGamma -nojpeg

EOF
cat > $d/ggH${MH}_012j_NLO_FXFX_HtoElElGamma_run_card.dat <<EOF
#*******************                                                 
# Running parameters
#*******************                                                 
#
