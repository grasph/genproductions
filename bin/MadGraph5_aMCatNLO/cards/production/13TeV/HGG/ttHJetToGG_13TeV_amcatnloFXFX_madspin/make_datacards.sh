...
d=ttHJetToGG_M${MH}_13TeV_amcatnloFXFX_madspin
mkdir $d

cat > $d/tth01j_5f_ckm_NLO_FXFX_MH${MH}_customizecards.dat <<EOF
set param_card mass 6 172.5
set param_card yukawa 6 172.5
set param_card mass 25 ${MH}.
EOF

cat > $d/tth01j_5f_ckm_NLO_FXFX_MH${MH}_madspin_card.dat <<EOF
...
cat > $d/tth01j_5f_ckm_NLO_FXFX_MH${MH}_proc_card.dat <<EOF
import model loop_sm-ckm_no_b_mass

generate p p > t t~ h [QCD] @0
add process p p > t t~ h j [QCD] @1

output tth01j_5f_ckm_NLO_FXFX_MH${MH} -nojpeg
EOF

cat > $d/tth01j_5f_ckm_NLO_FXFX_MH${MH}_run_card.dat <<EOF
#*******************                                                 
# Running parameters
#*******************                                                 
#
